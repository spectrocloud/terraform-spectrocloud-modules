  ...
  infra-pack-params-replaced = { for v in flatten([
    for k, v in local.cluster_infra_profiles_map : [
      for p in try(v.packs, []) : {
        name = format("%s%%%s-%s-%s", k, v.name, coalesce(v.version, "1.0.0"), p.name)
        value = join("\n", [
          for line in split("\n", try(p.is_manifest_pack, false) ?
            element([for x in local.cluster-profile-pack-map[format("%s%%%s-%s", v.name, coalesce(v.version, "1.0.0"), p.name)].manifest : x.content if x.name == p.manifest_name], 0) :
          local.cluster-profile-pack-map[format("%s%%%s-%s", v.name, coalesce(v.version, "1.0.0"), p.name)].values) :
          format(
    ...
    for k, v in local.cluster_addon_profiles_map : [
      for e in v : [
        for p in try(e.packs, []) : {
          ...
          value = join("\n", [
            for line in split("\n", try(p.is_manifest_pack, false) ?
              element([for x in local.cluster-profile-pack-map[format("%s%%%s-%s", e.name, coalesce(e.version, "1.0.0"), p.name)].manifest : x.content if x.name == p.manifest_name], 0) :
            local.cluster-profile-pack-map[format("%s%%%s-%s", e.name, coalesce(e.version, "1.0.0"), p.name)].values) :
            format(
              replace(line, "/%(${join("|", keys(p.params))})%/", "%s"),
              [
                for value in flatten(regexall("%(${join("|", keys(p.params))})%", line)) :
